...
> 虽说CSS学习来成本不高，但是内部机制还是要了解了解。如果你心(yan)思(jing)细(bu)腻(xia)会发现博客里有些只有标题，有两个原因：1、
> 内容是对标题的`简单`阐述；2、自己理解不到位，以后都理解了再添加内容。
...
终于可以写结尾了，重点是VFM、BFC、各种盒子，后续内容抽空再写，歇会儿。
